...
class finalizarPedido {
    ...
    public $sessao = "l9pl5sag3ho56ktamlm1nj7af6";
    ...
    public function listar() {
        $this->sqlConsultaCesta = "SELECT cesit.pro_id, cesit.cite_qtd, cesit.cite_valor, 
                                      ...
                                      FROM cesta_itens cesit, 
                                           ...
                                           clientes cli, 
                                           cidades cid
                                     WHERE cesit.ces_sessao = '$this->sessao'
    ...
    public function listarDadosPedido() {
        $this->sql = "SELECT cesta.cli_id, 
                               cli.cli_endereco, cli.cli_bairro, cli.cli_cep, cli.cid_id, cli.cli_fone1,
                               cid.cid_nome, cid.cid_uf,
                               ped.ped_total, ped.ped_frete, ped.ped_tipofrete
                          FROM cesta, 
                               clientes cli, 
                               cidades cid,
                               pedidos ped
                         WHERE cesta.ces_sessao = '$this->sessao'
                           AND cesta.cli_id = cli.cli_id
                           AND cli.cid_id = cid.cid_id
                           AND ped.cli_id = cesta.cli_id";
        $this->res = $this->bd->Execute($this->sql);
        $cidadeUF = $this->res->fields['cid_nome'] . " - " . $this->res->fields['cid_uf'];
        $valorTotalPedido = $this->res->fields['ped_total'] + $this->res->fields['ped_frete'];
        //return $this->res;
        $retorno = array($this->res->fields['cli_endereco'], $this->res->fields['cli_bairro'], $cidadeUF, $this->res->fields['cli_cep'], $this->res->fields['cli_fone1'], $this->res->fields['ped_total'], $this->res->fields['ped_frete'], $valorTotalPedido);
    ...
    public function verificarCartao($nome, $num_cartao, $mes_val_cartao, $ano_val_cartao, $cod_seguranca) {

        function luhn_check($num_cartao) {

            // Strip any non-digits (useful for credit card numbers with spaces and hyphens)
            $num_cartao = preg_replace('/\D/', '', $num_cartao);

            // Set the string length and parity
            $number_length = strlen($num_cartao);
        ...
        if (luhn_check($num_cartao) == TRUE) {
            $retorno = "CartaoValido";
        } else if (luhn_check($num_cartao) == FALSE){
            $retorno = "CartaoInvalido";
        }

        //$retorno = array($nome, $num_cartao, $mes_val_cartao, $ano_val_cartao, $cod_seguranca);
        //echo var_dump($retorno);
        return json_encode($retorno);
